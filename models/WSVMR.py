...
class WSVMR(nn.Module):
    def __init__(self, config):
        ...
        self.hard_mask = config['hard_mask']
    ...
    def forward_train(self, frames_feat, frames_len, words_id, words_feat, words_len, weights, glance, **kwargs):
        ...
        pos_vid_concept = self.vid_encoder(frames_feat, frames_mask, gauss_weight=None, require_enc=False)
    ...
    def forward_rank_train(self, center, width, frames_feat, frames_len, words_id, words_feat, words_len, weights, glance, **kwargs):
        bsz, n_frames, _ = frames_feat.shape
        vid_pred_vec = self.vid_pred_vec.view(1, 1, -1).expand(bsz, 1, -1)
        ...
        gauss_center = center.reshape(-1)
        gauss_width = width.reshape(-1)
        ...
        if self.hard_mask:
            props_mask = gauss_weight.masked_fill(gauss_weight>0, 1)
            pos_vid_concept = self.vid_encoder(props_feat, props_mask)
        else:
            pos_vid_concept = self.vid_encoder(props_feat, props_mask, gauss_weight=pos_weight)
    ...
    def forward_vid(self, frames_feat, frames_len, **kwargs):
        ...
        pos_vid_concept = self.vid_encoder(frames_feat, frames_mask, gauss_weight=None)
        h = self.fc_gauss(pos_vid_concept).reshape(bsz, self.num_props, 2).sigmoid()
        gauss_center = h[:, :, 0].reshape(-1)
        gauss_width = h[:, :, 1].reshape(-1)
    ...
    def forward_vid_rank(self, center, width, frames_feat, frames_len, **kwargs):
        bsz, n_frames, _ = frames_feat.shape
        vid_pred_vec = self.vid_pred_vec.view(1, 1, -1).expand(bsz, 1, -1)
    ...
    def _mask_words(self, words_feat, words_len, weights=None):
        ...
        for i, l in enumerate(words_len):
            l = int(l)
            num_masked_words = max(l//self.masked_ratio, 1) 
